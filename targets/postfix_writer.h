...
namespace udf {
  ...
  public:
    postfix_writer(std::shared_ptr<cdk::compiler> compiler, cdk::symbol_table<udf::symbol> &symtab, cdk::basic_postfix_emitter &pf) :
        basic_ast_visitor(compiler), _symtab(symtab), _errors(false), _inFunctionArgs(false), _inFunctionBody(false), 
          _inForInit(false), _function(nullptr), _offset(0), _lvalueType(cdk::TYPE_VOID), _currentFunctionName(""),
             _currentBodyRetLabel(""), _pf(pf), _lbl(0){
    }
  ...
  protected:
    void tensor_data(cdk::sequence_node *data, std::vector<cdk::expression_node *> &flat);
  public:
