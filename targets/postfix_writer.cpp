...
void udf::postfix_writer::do_double_node(cdk::double_node * const node, int lvl) {
  if (_inFunctionBody) {
    _pf.DOUBLE(node->value()); // double literal is on the stack: push a double
  } else {
    _pf.SDOUBLE(node->value()); // double literal is on the DATA segment
  }
}
...
void udf::postfix_writer::do_unary_minus_node(cdk::unary_minus_node * const node, int lvl) {
  ...
  if (node->argument()->is_typed(cdk::TYPE_TENSOR)) {
    ...
    _pf.DNEG(); // negate a double
  } 
  else if (node->argument()->is_typed(cdk::TYPE_INT)) {
    node->argument()->accept(this, lvl + 2); // determine the value
    _pf.NEG(); // negate an integer
  }
}
...
void udf::postfix_writer::do_add_node(cdk::add_node * const node, int lvl) {
  ...
  if (node->type()->name() == cdk::TYPE_TENSOR){
    if (node->left()->is_typed(cdk::TYPE_TENSOR) && node->right()->is_typed(cdk::TYPE_TENSOR)) {
      ...
      node->right()->accept(this, lvl + 2); // determine the value
      if ((node->right()->type()->name() == cdk::TYPE_INT)) {
        _pf.I2D();
      }
      node->left()->accept(this, lvl + 2); //tensor
      ...
      node->left()->accept(this, lvl + 2); // determine the value
...
void udf::postfix_writer::do_rvalue_node(cdk::rvalue_node * const node, int lvl) {
  ...
  if (node->type()->name() == cdk::TYPE_DOUBLE) {
    ...
    // integers, pointers, and strings, tensors(pointers)
    _pf.LDINT();
  }
}
...
void udf::postfix_writer::do_return_node(udf::return_node * const node, int lvl) {
  ...
  if (_function->type()->name() != cdk::TYPE_VOID) {
    ...
    if (_function->type()->name() == cdk::TYPE_INT || _function->type()->name() == cdk::TYPE_POINTER 
...
void udf::postfix_writer::do_function_definition_node(udf::function_definition_node * const node, int lvl) {
  ...
  if (node->arguments()->size() > 0) {
    _inFunctionArgs = true; // remember this for local declarations
    ...
    _inFunctionArgs = false; // remember this for local declarations
  }
  ...
  _pf.LABEL(_function->name()); // function label
  ...
  _pf.LABEL(_currentBodyRetLabel); // return jumps here 
  _pf.LEAVE();
  _pf.RET();
...
void udf::postfix_writer::do_function_call_node(udf::function_call_node * const node, int lvl) {
  ...
  if (symbol->is_typed(cdk::TYPE_INT) || symbol->is_typed(cdk::TYPE_POINTER) || symbol->is_typed(cdk::TYPE_STRING) 
      || symbol->is_typed(cdk::TYPE_TENSOR)) {
    _pf.LDFVAL32();
  } 
...
void udf::postfix_writer::do_for_node(udf::for_node * const node, int lvl) {
  ...
  node->iteration()->accept(this, lvl + 2);
...
void udf::postfix_writer::do_sizeof_node(udf::sizeof_node * const node, int lvl) {
  ...
  if(node->argument()->type()->name() == cdk::TYPE_TENSOR) {

    node->argument()->accept(this, lvl + 2);
    //get dims
    _functions_to_declare.insert("tensor_size");
    _pf.CALL("tensor_size");
    _pf.TRASH(4); // 4 bytes for the pointer to the tensor data
    _pf.LDFVAL32();
    _pf.INT(8); // size of pointer to the tensor
    _pf.MUL(); // multiply by size of pointer to the tensor
  } 
  else {
...
void udf::postfix_writer::do_index_node(udf::index_node * const node, int lvl) {
  ASSERT_SAFE_EXPRESSIONS;
  // index node is a pointer to an array, so we need to load the base address
  if (node->base()) {
  ...
  if (_inFunctionBody) {
    if (node->init()) {
      ...
      if (node->is_typed(cdk::TYPE_INT) || node->is_typed(cdk::TYPE_STRING) || node->is_typed(cdk::TYPE_POINTER) ) {
        ...
        _pf.LOCAL(symbol->offset());
        _pf.STINT(); // store the pointer to the tensor in the local variable
      }
      else {
      ...
      for (int i = tensor_type->n_dims() - 1; i >= 0; --i)
        _pf.INT(tensor_type->dim(i));
      _pf.INT(tensor_type->n_dims());

      _functions_to_declare.insert("tensor_zeros");
      _pf.CALL("tensor_zeros");
      _pf.TRASH(4 * tensor_type->n_dims()); // 4 bytes for each dimension
      _pf.TRASH(4);
      _pf.LDFVAL32();
      _pf.DUP32();
      _pf.LOCAL(symbol->offset()); //it just works
      _pf.STINT();
    }
  }
  else{
    if (!_function) { //global variable 
      if (node->init() == nullptr) { // no assignment
        if (!node->is_typed(cdk::TYPE_TENSOR))
          _pf.BSS();
        else 
          _pf.DATA(); // tensors are always initialized
        _pf.ALIGN();
        ...
        if (node->is_typed(cdk::TYPE_TENSOR)) {
          // global tensors cant be initialized easily
        }
        _pf.SALLOC(typesize);
      } 
        ...
        if (node->is_typed(cdk::TYPE_INT) || node->is_typed(cdk::TYPE_DOUBLE) || node->is_typed(cdk::TYPE_POINTER) 
          ...
          if (node->is_typed(cdk::TYPE_INT) || node->is_typed(cdk::TYPE_POINTER) || node->is_typed(cdk::TYPE_STRING)) 
            ...
            if (node->init()->is_typed(cdk::TYPE_DOUBLE)) {
              node->init()->accept(this, lvl);
            } 
            else if (node->init()->is_typed(cdk::TYPE_INT)) {
              cdk::integer_node *dclini = dynamic_cast<cdk::integer_node*>(node->init());
...
void udf::postfix_writer::tensor_data(cdk::sequence_node *data, std::vector<cdk::expression_node *> &flat) {
  for (size_t i = 0; i < data->size(); ++i) {
    auto expr = dynamic_cast<cdk::expression_node *>(data->node(i));
    if (expr->type()->name() == cdk::TYPE_TENSOR) {
      // recurse into nested tensor_node
      auto tensor = dynamic_cast<udf::tensor_node *>(expr);
      tensor_data(tensor->fields(), flat);
    } 
    else {
      flat.push_back(expr);
    }
...
void udf::postfix_writer::do_tensor_node(udf::tensor_node * const node, int lvl) {
  ...
  _pf.TRASH(4 * tensor_type->n_dims());
  _pf.TRASH(4); // 4 bytes for the pointer to the tensor data
  _pf.LDFVAL32();

  std::vector<cdk::expression_node *> data;
  tensor_data(node->fields(), data);

  for (size_t i = 0; i < data.size(); ++i) {
    cdk::expression_node *data_field = data[i];

    _pf.INT(i); // 4 

    data_field->accept(this, lvl);
    if (data_field->is_typed(cdk::TYPE_INT))
      _pf.I2D(); // 8
...
void udf::postfix_writer::do_tensor_capacity_node(udf::tensor_capacity_node * const node, int lvl) {
  //corresponde ao número de células do tensor
  ASSERT_SAFE_EXPRESSIONS;
...
void udf::postfix_writer::do_tensor_rank_node(udf::tensor_rank_node * const node, int lvl) {
  ...
  _pf.TRASH(4); // 4 bytes for the pointer to the tensor data
  _pf.LDFVAL32(); // store the number of dimensions in the stack
}
...
void udf::postfix_writer::do_tensor_dim_node(udf::tensor_dim_node * const node, int lvl) {
  ...
  _pf.TRASH(8); // 4 bytes for the pointer to the tensor data
  _pf.LDFVAL32(); // store the size of the dimension in the stack
}
...
void udf::postfix_writer::do_tensor_contraction_node(udf::tensor_contraction_node * const node, int lvl) {
  ...
  _pf.TRASH(8); // 4 bytes for each tensor pointer
  _pf.LDFVAL32();
}
...
void udf::postfix_writer::do_tensor_reshape_node(udf::tensor_reshape_node * const node, int lvl) {
  ...
  _pf.TRASH(4 * node->arguments()->size()); // 4 bytes for each dimension
  _pf.TRASH(8); // 4 bytes for the pointer to the tensor data
  _pf.LDFVAL32();
}
